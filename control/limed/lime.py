...
def creditable(targets=None):
    """Everything that carries attribution: targets and scenarios both. A
    scenario is packaged work with an author and a licence in its manifest
    like anything else, and it was being skipped by the credits page and by
    doctor, so a scenario could arrive with no author and nothing would say
    so."""
    items = dict(targets if targets is not None else load_targets())
    for slug, sc in load_scenarios().items():
        key = slug if slug not in items else f"scenario:{slug}"
        items[key] = {**sc, "kind": "scenario"}
    return items
...
def _credits_markdown(targets):
    ...
    for kind in sorted(by_kind):
        what = "scenario" if kind == "scenario" else "target"
        out += [f"### {kind}", "", f"| {what} | author | licence | source |", "|---|---|---|---|"]
...
def cmd_credits(targets, args):
    ...
    if getattr(args, "markdown", False):
        print(_credits_markdown(creditable(targets)))
        return
    print("limeyard runs other people's work. Credit where it is due.\n")
    targets = creditable(targets)
    by_kind = {}
    for t in targets.values():
        by_kind.setdefault(t.get("kind", "?"), []).append(t)
    for kind in sorted(by_kind):
...
def cmd_doctor(targets, args):
    ...
    # Attribution is a hard gate: nothing may exist here without crediting its
    # author, scenarios included.
    for t in creditable(targets).values():
        up = t.get("upstream") or {}
        manifest = "scenario.yml" if t.get("kind") == "scenario" else "target.yml"
        if not up.get("author"):
            print(col(f"[!!] {t['slug']}: {manifest} has no upstream.author "
